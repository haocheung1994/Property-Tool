		...
		try {
			...
			if(!eventType){ //信息交互事件
				...
				if(textMsg.getEvent().equals("subscribe")){ //关注事件
					System.out.println("zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzfsdfsfsdf");
					...
					item.setPicUrl(wxSetting.getLink()+"/ad.jpg");	//图片链接
					item.setUrl(wxSetting.getLink()+clickText.getUrl());        //链接指向
					articleList.add(item);
					//replyMsg.append(clickText.getContent());
