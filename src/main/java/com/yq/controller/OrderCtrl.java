...
//	WechatPushMassage wechatPushMassage = new WechatPushMassage();
	...
	public String insert(String goods_id, String goods_name, String goods_img, String goods_spe, String goods_price,
			String goods_num, Float goods_total, Integer goods_total_num,  String cps_name,
			@RequestParam(defaultValue = "0") Float cps_price, String addr_name, String receive, String oppen_id,
			Integer status, String note, HttpSession session) {
		...
		final String finalGoods_name = goods_name;
		final String finalAddr_name = addr_name;
		String [] arr = addr_name.split("\\s+");


		Map<String,Map> orderInfo=new HashMap<String,Map>() {
			{
				...
				put("keyword2", new HashMap<String, String>() {
					{
						put("value", finalGoods_name+"（共"+goods_num+"个)"+ "\n");
					}
				});
				put("keyword3", new HashMap<String, String>() {
					{
						put("value", goods_price +"元"+ "\n");
					}
				});
		...
		String openid="oyrZEvychDVA-Qf1Ph7gJ7-rVIns";
		String Template_id="ADDa2ctbGrQZFj3wwsNKX0CmFDagpLoxY8dm1BRL9YE";
		String url=wxSettingService.selectByPrimaryKey(1).getLink()+"/page/index.html";
		templateMessageService.toJobFinder(openid,Template_id,url,orderInfo);
...
//			wechatPushMassage.pushMessage(map);
	...
	@RequestMapping(value = "/page/cancel_order.html")
	public void cancel_order(Integer orderid) {
	...
	@RequestMapping(value = "/page/unpay.html")
	public ModelAndView unpay(@RequestParam(defaultValue = "-2") Integer status, String oppen_id, HttpSession session,HttpServletRequest request) {
		// setOppen_id("111", session);// 测试代码，模仿登录
		order.setOppen_id(getOppen_id(session));
		order.setStatus(-2);
		order.setStart_time("");
		...
		request.setAttribute("slist0", list0.size());
		map.put("list0", list0);
		ModelAndView ml = new ModelAndView();
		ml.addObject("map", map);
		ml.setViewName("page/order-list1");
		return ml;
	}

	@RequestMapping(value = "/page/unfahuo.html")
	public ModelAndView unfahuo(@RequestParam(defaultValue = "-2") Integer status, String oppen_id, HttpSession session,HttpServletRequest request) {
		// setOppen_id("111", session);// 测试代码，模仿登录
		order.setOppen_id(getOppen_id(session));
		order.setStatus(-2);
		order.setStart_time("");
		...
		ModelAndView ml = new ModelAndView();
		ml.addObject("map", map);
		request.setAttribute("slist1", list1.size());
		ml.setViewName("page/order-list2");
		return ml;
	}


	@RequestMapping(value = "/page/daishouhuo.html")
	public ModelAndView daishouhuo(@RequestParam(defaultValue = "-2") Integer status, String oppen_id, HttpSession session,HttpServletRequest request) {
		// setOppen_id("111", session);// 测试代码，模仿登录
		order.setOppen_id(getOppen_id(session));
		order.setStatus(-2);
		order.setStart_time("");
		...
		ModelAndView ml = new ModelAndView();
		ml.addObject("map", map);
		request.setAttribute("slist2", list2.size());
		ml.setViewName("page/order-list3");
		return ml;
	}
