...
<body>
	<nav class="breadcrumb">
		...
		供应商查看 <a class="btn btn-success radius r mr-20"
				...
				<tbody>
					<c:forEach items="${suplist}" var="list"   >
						<tr class="text-c">
							...
							<td>${list.supTypeInAcc.suptypename}</td>
							<td>${list.suptel}</td>
							<td>${list.supcity}</td>
							<td>${list.supaddress}</td>
							<td>
							<a href="cpsListById.html?cps_id=${list.supplierid}">编辑</a>&nbsp;&nbsp;
							<a href="javascript:;" onclick="del('${list.supplierid}')">删除</a></td>
						</tr>
					</c:forEach>
